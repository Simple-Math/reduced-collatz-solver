...
\( d \in \{0, \dots, 9\} \), and directed edges represent possible last-digit transitions under a single Collatz step.

For odd values of \( n \), the transition is uniquely determined by the map \( n \mapsto 3n + 1 \). For even values of \( n \), the transition depends on the prefix parity of \( n \), resulting in two possible outgoing edges.
